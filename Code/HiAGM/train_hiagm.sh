#!/bin/bash
#SBATCH --job-name=train_hiagm           # Name of the job
#SBATCH --output=logs/hiagm_out_%j.txt   # Standard output log (%j = job ID)
#SBATCH --error=logs/hiagm_err_%j.txt    # Standard error log
...
python $HOME/scripts/train_hiagm.py \
    --config $HOME/configs/hiagm_config.json \
