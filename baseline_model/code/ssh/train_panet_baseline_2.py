...
from transformers import AutoModel, Trainer, TrainingArguments, AutoTokenizer, DataCollatorWithPadding, AutoModelForSequenceClassification, set_seed
import numpy as np
...
# 3. Model Definition with Mean Pooling
...
#     # We use np.divide with 'where' to safely handle 0/0
#     precision_samples = np.divide(tp, pred_count, out=np.zeros_like(tp, dtype=float), where=pred_count > 0)
#     recall_samples = np.divide(tp, true_count, out=np.zeros_like(tp, dtype=float), where=true_count > 0)

#     # Calculate F1 per sample
#     denom = precision_samples + recall_samples
#     f1_samples = np.divide(2 * precision_samples * recall_samples, denom, 
...
def run_experiment(seed):
    ...
    # # Reload best Phase 1 weights into the model
    # best_ckpt = trainer.state.best_model_checkpoint
    # state_dict = torch.load(f"{best_ckpt}/pytorch_model.bin", map_location='cpu')
